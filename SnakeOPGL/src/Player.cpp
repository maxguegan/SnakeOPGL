...
Player::Player(Texture& headTexture, Texture& bodyTexture, Texture& tailTexture, glm::ivec2 pos, std::vector<std::vector<Tile>>& map)
	: headTexture(headTexture), bodyTexture(bodyTexture), tailTexture(tailTexture),headPos(pos),map(map), curDir(GAUCHE), nextDir(GAUCHE)
{

	for (int i = 0; i < STARTSIZE + 1; i++) {
		this->position[i] = glm::ivec2(headPos.x + i, headPos.y);
		map[headPos.y][headPos.x + i].state = SNAKE;
		this->body.push_back(GameObject(bodyTexture, map[headPos.y][headPos.x + i].position, glm::vec2(20.0f)));
	}
